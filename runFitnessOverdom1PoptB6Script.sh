...
while [ $currentTfValue -le $finalTf0value ]; do  # -le is less than or equal to
        echo "currentTfValue="$currentTfValue   #" nextTFvalue="$nextTFvalue
	bsub -q long -n 16 -W 72:00 -R span[hosts=1] -R rusage[mem=16]  /home/ap73a/simulations/fitnessOverdomOptGtypeByPopt/fitnessOverdomOptGtypeByPopt "$bitstringLen" "$Ntfsat" "$omega" "$Popt" "$Popt" "$PoptTotalSteps" "$stepSize" "$currentTFvalue" "$currentTFvalue"
#	echo "currentTfValue="$currentTfValue" nextTFvalue="$nextTFvalue
#	let currentTfValue=currentTfValue+tf0valuesPerOutputFile
        let currentTfValue=currentTfValue+1
#        let nextTFvalue=nextTFvalue+tf0valuesPerOutputFile   
	done
